...
function insertTableRows(data) {
  ...
  data.forEach(row => {
    ...
    downButton.addEventListener("click", () => onMatchVote(voteSpan, row, -1))
    upButton.addEventListener("click", () => onMatchVote(voteSpan, row, 1))
...
async function searchByName(name) {
  ...
  if (!includeAll.checked && !hasVoted) {
    data = tableData.filter(character => character.name.toLowerCase().includes(name.toLowerCase())
